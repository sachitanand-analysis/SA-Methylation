...
# ============================================================
# 3b. ANNOTATION CONFIDENCE
# ============================================================

# ---------- ALL DMPs ----------

all_DMPs_anno$Annotation_Confidence <- "Low"

# Medium confidence first
all_DMPs_anno$Annotation_Confidence[
  grepl(
    "TSS1500|5'UTR|1stExon|5UTR|exon_1",
    all_DMPs_anno$UCSC_RefGene_Group
  )
] <- "Medium"
...
all_DMPs_anno$Annotation_Confidence[
  grepl(
    "TSS200|1stExon|5'UTR|5UTR|exon_1",
    all_DMPs_anno$UCSC_RefGene_Group
  ) &
  all_DMPs_anno$Relation_Primary == "Island"
] <- "High"

# ---------- SIGNIFICANT DMPs ----------

sig_DMPs_anno$Annotation_Confidence <- "Low"

# Medium confidence first
sig_DMPs_anno$Annotation_Confidence[
  grepl(
    "TSS1500|5'UTR|1stExon|5UTR|exon_1",
    sig_DMPs_anno$UCSC_RefGene_Group
  )
...
sig_DMPs_anno$Annotation_Confidence[
  ...
  sig_DMPs_anno$Relation_Primary == "Island"
] <- "High"
...
write.table(sig_DMPs_anno,
            file.path(outDir, "Significant_DMPs_annotated.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)
cat("Significant DMPs:", nrow(sig_DMPs_anno), "\n")
...
print(reg_summary)
cat("Predicted Activation:",
    sum(sig_DMPs_anno$Regulation == "Predicted Activation"), "\n")
cat("Predicted Silencing:",
    sum(sig_DMPs_anno$Regulation == "Predicted Silencing"), "\n")
cat("Unclear:",
