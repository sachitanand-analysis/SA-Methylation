...
if (!is.null(kidney_bp) && nrow(as.data.frame(kidney_bp)) > 0) {
  kidney_bp <- clusterProfiler::simplify(
    kidney_bp, cutoff = 0.7, by = "p.adjust", select_fun = min
  )
}
if (!is.null(kidney_cc) && nrow(as.data.frame(kidney_cc)) > 0) {
  kidney_cc <- clusterProfiler::simplify(
    kidney_cc, cutoff = 0.7, by = "p.adjust", select_fun = min
  )
}
if (!is.null(kidney_mf) && nrow(as.data.frame(kidney_mf)) > 0) {
  kidney_mf <- clusterProfiler::simplify(
    kidney_mf, cutoff = 0.7, by = "p.adjust", select_fun = min
  )
}

cat("\n=== Post-simplify() kidney-filtered GO term counts ===\n")
cat("Kidney BP terms :",
