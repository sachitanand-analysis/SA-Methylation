...
activated_genes <- sig_DMPs_anno %>%
  filter(Regulation == "Predicted Activation") %>%
  arrange(adj.P.Val) %>%
...
silenced_genes <- sig_DMPs_anno %>%
  filter(Regulation == "Predicted Silencing") %>%
  arrange(adj.P.Val) %>%
  mutate(Gene = sapply(strsplit(UCSC_RefGene_Name, ";"),
                        `[`, 1)) %>%
  filter(!is.na(Gene) & Gene != "") %>%
...
plot_reg <- bind_rows(
  mutate(activated_genes, Regulation="Predicted Activation"),
  mutate(silenced_genes,  Regulation="Predicted Silencing")
)
...
if (nrow(plot_reg) > 0) {
  ...
  p_reg_scatter <- ggplot(
    ...
    scale_color_manual(
      values=c("Predicted Activation"="#2166AC",
               "Predicted Silencing"="#B2182B"),
      name=NULL
    ) +
  ...
  ggsave(file.path(outDir, "S2.png"),
...
cat("  Predicted_Activation_Genes.csv\n")
cat("  Predicted_Silencing_Genes.csv\n")
...
sample_map <- c(
  ...
  "MC53/24" = "S17", "MC56/24" = "S19", "MC60/24" = "S20"
)
...
legend("topright",
       legend   = c("Control (S1–S20)", "PUV (S21–S40)"),
...
title(
  main = "Hierarchical Clustering of Global DNA Methylation",
  cex.main = 1.0
)
...
ggsave(file.path(outDir, "S1D.png"),
